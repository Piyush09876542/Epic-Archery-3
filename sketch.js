...
function keyPressed() {
  if(keyCode === DOWN_ARROW){
    ...
    var angle = playerArcher
    var arrow = new PlayerArrow(posX, posY, 100, 10);
    arrow.trajectory = [];
    Matter.Body.setAngle(arrows.body);
    playerArrows.push(arrow);
  }
  function showArrows(arrow, index) { arrow.display();
    if (arrow.body.position.x >= width || arrow.body.position.y >= height - 50) 
    { Matter.World.remove(world, arrow.body); arrows.splice(index, 1); } }
